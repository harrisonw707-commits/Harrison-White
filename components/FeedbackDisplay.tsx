  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in slide-in-from-bottom duration-700 text-slate-100 pb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-slate-800 pb-8 gap-6">
        <div>
          <h2 className="text-4xl font-black text-slate-50">{session.role} <span className="text-yellow-400">Analysis</span></h2>
        ...
        <div className="text-left md:text-right">
           <div className="text-5xl font-black text-yellow-400">{feedback.score}<span className="text-xl text-slate-700">/100</span></div>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
           <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 overflow-hidden shadow-2xl">
             ...
             {session.videoUrl ? (
               ...
               <div className="aspect-video bg-black flex flex-col items-center justify-center p-6 border border-slate-800 border-dashed rounded-2xl opacity-40">
                ...
                {session.transcription.length > 0 ? (
                  session.transcription.map((line, i) => (
                    <div key={i} className={`text-[11px] leading-relaxed p-3 rounded-xl border ${line.startsWith('You:') ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-slate-800/40 border-slate-800'}`}>
                      <span className={`font-bold uppercase tracking-widest ${line.startsWith('You:') ? 'text-emerald-400' : 'text-yellow-400'}`}>{line.split(':')[0]}</span>
        ...
        <div className="lg:col-span-8 space-y-8">
             ...
             <div className="grid md:grid-cols-2 gap-12">
               <div className="h-64 flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                      ...
                      <Radar name="Score" dataKey="A" stroke="#facc15" fill="#facc15" fillOpacity={0.4} />
                    </RadarChart>
                  </ResponsiveContainer>
               </div>
               <div className="space-y-4">
                 <div className="text-[11px] text-slate-400 leading-relaxed bg-slate-800/20 p-6 rounded-2xl border border-slate-800 shadow-inner">
                   <div className="text-yellow-400 font-black uppercase tracking-tighter text-[10px] mb-2 flex items-center">
                 ...
                 <div className="grid grid-cols-2 gap-4 pt-2">
                    <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-800">
                       ...
                       <div className="text-xs font-black text-yellow-400">{feedback.metrics.pacing.label}</div>
                    </div>
                    <div className="bg-slate-800/40 p-3 rounded-xl border border-slate-800">
          ...
          <div className="bg-yellow-400/5 border border-yellow-400/20 rounded-[2.5rem] p-10 relative overflow-hidden">
            <div className="absolute top-0 right-0 -mt-10 -mr-10 w-64 h-64 bg-yellow-400/5 blur-[100px] rounded-full" />
            ...
            <div className="relative z-10">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-3xl flex flex-col justify-between group hover:border-yellow-400/40 transition-all cursor-pointer" onClick={onRestart}>
                  <div>
                    <div className="w-10 h-10 bg-yellow-400/20 rounded-2xl flex items-center justify-center text-yellow-400 mb-4 group-hover:scale-110 transition-transform">
                  ...
                  <div className="mt-6 flex items-center text-[10px] font-black text-yellow-400 uppercase tracking-widest group-hover:translate-x-1 transition-transform">
      ...
      <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-6 pt-12 border-t border-slate-900">
        ...
        <button onClick={() => window.print()} className="px-12 py-4 bg-yellow-400 text-black rounded-2xl font-bold hover:bg-yellow-500 transition-all text-sm shadow-xl shadow-yellow-400/20 flex items-center justify-center space-x-2">
